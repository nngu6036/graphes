...
def test_refiner_materializes_all_predicted_molecular_attributes():
    topology = nx.path_graph(3)
    endpoint = topology.copy()
    for node, atomic_number in enumerate((6, 8, 7)):
        endpoint.nodes[node]["atomic_num"] = atomic_number
    endpoint.edges[0, 1]["bond_type"] = 2
    endpoint.edges[1, 2]["bond_type"] = 1
    vocabulary = GraphCategoryVocabulary(
        node_values=(6, 7, 8, 9),
        edge_values=(1, 2, 3, 4),
    ...
    refined = refine_graph_with_hybrid_predictions(
        topology,
        model=None,
        vocabulary=vocabulary,
        graphlet_basis=GraphletBasis.from_config(summary_config),
        ...
        prediction_fn=lambda *_args, **_kwargs: prediction,
    )

    assert _edge_set(refined) == _edge_set(topology)
    assert [
        refined.nodes[node]["atomic_num"] for node in sorted(refined.nodes())
    ] == [6, 8, 7]
    assert refined.edges[0, 1]["bond_type"] == 2
    assert refined.edges[1, 2]["bond_type"] == 1
