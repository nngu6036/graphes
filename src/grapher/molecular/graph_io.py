    ...
    *,
    ...
    for atom in mol.GetAtoms():
        ...
        if atomic_num not in allowed_atoms:
            raise ValueError(
                f"Atom {atomic_num} is outside allowed atom set {allowed_atoms}."
            )
        idx = int(atom.GetIdx())
    ...
    for bond in mol.GetBonds():
        ...
        if bond_type not in allowed_bonds:
            raise ValueError(
                f"Bond type {bond_type} is outside allowed set {allowed_bonds}."
            )
        g.add_edge(
            u,
            v,
            bond_type=bond_type,
            bond_order=float(bond_type if bond_type != BOND_AROMATIC else 1.5),
        )
...
def read_smiles_file(
    path: str | Path, *, smiles_column: str | int | None = None
) -> list[str]:
    ...
    if path.suffix.lower() in {".csv", ".tsv"} and smiles_column is not None:
        ...
        with path.open("r", encoding="utf-8") as f:
            ...
            return [
                row[str(smiles_column)].strip()
                for row in reader
                if row.get(str(smiles_column))
            ]
    ...
    *,
    ...
    for smi in smiles:
        try:
            ...
            graph = mol_to_nx(
                mol, remove_h=remove_h, kekulize=kekulize, allowed_atoms=allowed_atoms
            )
...
def split_graphs(
    graphs: list[nx.Graph], *, seed: int, train_frac: float = 0.8, val_frac: float = 0.1
):
