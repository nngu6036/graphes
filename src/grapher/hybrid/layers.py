...
class EdgeAwareMPNNLayer(nn.Module):
    """Permutation-equivariant message passing over dense pair states."""
    ...
    def forward(
        ...
        aggregated = (
            messages.sum(dim=2)
            / adjacency.sum(dim=2, keepdim=True).clamp_min(1).float()
        )
        node_state = self.node_norm(
            node_state + self.node_update(torch.cat([node_state, aggregated], dim=-1))
        )
