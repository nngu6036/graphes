...
def _choose_action(actions: list[Action], deltas: np.ndarray, cfg: RefinerConfig, rng: np.random.Generator) -> tuple[Action | None, float]:
    if not actions:
        return None, 0.0
...
def refine_graph(
    ...
    *,
    ...
    cfg = refiner_config if isinstance(refiner_config, RefinerConfig) else RefinerConfig.from_dict(refiner_config)
    ...
    for step in range(cfg.steps):
        ...
        if not candidates:
            trace.append({"step": step, "energy": current_energy, "accepted": False, "reason": "no_candidates"})
            break
        deltas: list[float] = []
        for action in candidates:
            candidate_graph = apply_action(g, action)
            candidate_energy = distance_to_summary(candidate_graph, target_summary, summary_config, energy_weights)
            deltas.append(current_energy - candidate_energy)
        action, chosen_delta = _choose_action(candidates, np.asarray(deltas, dtype=np.float64), cfg, generator)
        if action is None:
            trace.append(
                {
                    "step": step,
