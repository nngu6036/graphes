...
def sample_graphs(
    ...
    *,
    ...
    if model_key == "dhvae":
        checkpoint = Path(model_cfg.get("checkpoint_path") or "outputs/checkpoints/dhvae.pt")
    ...
    if model_key in {"grapher", "grapher_generic", "grapher_attributed"}:
        ...
        dhvae, _ = load_dhvae_checkpoint(dhvae_checkpoint, device=device)
        ...
        with progress_bar(total=int(num_graphs), desc=desc, unit="graph", enabled=show_progress) as update:
            ...
            while len(graphs) < int(num_graphs) and rounds < 20:
                ...
                batch, _ = grapher.generate(
                    ...
                    degree_temperature=float(model_cfg.get("degree_sample_temperature", model_cfg.get("dhvae_temperature", 1.0))),
