...
def generate_msvae_samples(
    *,
    ...
    if not checkpoint.exists():
        raise FileNotFoundError(f"MS-VAE checkpoint not found: {checkpoint}. Run scripts/train_msvae_model.py first.")
    ...
    rejected = {"not_graphical": 0, "not_connected_feasible": 0}
    attempts = 0
    ...
    with PeakMemoryMonitor() as memory_monitor:
        while len(accepted) < int(num_samples) and attempts < max_attempts:
            ...
            candidates = model.generate(batch_size)
            attempts += batch_size
    ...
    logger.info("Saved %d degree sequences to %s", len(accepted), out)
    return metadata


def main() -> None:
    parser = argparse.ArgumentParser(description="Generate degree sequences from a trained MS-VAE.")
