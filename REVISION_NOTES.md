...
- `scripts/train_msvae_model.py`: trains the size-conditioned DH-VAE degree prior on degree histograms and graph-size embeddings, saving a structured checkpoint.
...
- `scripts/generate_grapher_samples.py`: loads the DH-VAE/MS-VAE compatibility checkpoint and GraphER checkpoint, constructs connected canonical sources, and samples graph rewiring trajectories.
...
- `MSVAE` is now a compatibility alias for `DHVAE`; legacy independent-count MS-VAE checkpoints should be retrained.


## Size-conditioned DH-VAE degree prior update

- Replaced the legacy independent-count MS-VAE internals with the paper-aligned size-conditioned Degree-Histogram VAE while keeping the public class/script name `MSVAE` for checkpoint-path and workflow compatibility.
- `src/grapher/models/model_msvae.py` now provides `DHVAE`, explicit degree-zero histogram encoding, size embeddings, masked degree logits for `k < n`, multinomial histogram sampling, and a multinomial NLL reconstruction loss.
- `scripts/train_msvae_model.py` now trains on `(histogram, graph_size)` pairs, stores the empirical graph-size distribution in checkpoints, and records DH-VAE-specific metadata.
- `scripts/generate_msvae_samples.py` now samples graph sizes from the checkpoint's empirical size distribution and exposes `--temperature` for the DH-VAE categorical degree probabilities before multinomial sampling.
- `src/grapher/models/checkpoint.py` now requires DH-VAE checkpoints and gives an explicit retraining message for legacy MS-VAE checkpoints.
- Updated `configs/models/msvae.yaml`, `README.md`, and model registry aliases to document that `msvae` now means the size-conditioned DH-VAE compatibility path.
...
PYTHONPATH=src python scripts/train_msvae_model.py --dataset sbm --model-config /tmp/graphes_dhvae_smoke/dhvae_small.yaml --dataset-config /tmp/graphes_dhvae_smoke/sbm_small.yaml --dataset-root outputs_dhvae_smoke/datasets --device cpu --run-id 0
PYTHONPATH=src python scripts/generate_msvae_samples.py --dataset sbm --model-config /tmp/graphes_dhvae_smoke/dhvae_small.yaml --num-samples 4 --device cpu --run-id 0 --force --max-attempts 200
```
...
- It also supports MS-VAE degree-sequence pickle statistics through `--input-pkl`.
